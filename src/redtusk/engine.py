...
# ---------------------------------------------------------------------------
# rmeta → detonator contract conversion helpers
# ---------------------------------------------------------------------------
...
def _run_worker(
    ...
    *,
    ...
    into *rmeta_outdir*.  The detonator harness writes its own ``metadata.json``
    ...
    with tempfile.TemporaryDirectory() as scratch_base:
        ...
        # output files.  The harness writes the detonator envelope to the
...
class RedTuskEngine:
    """Detonator Engine wrapping the RedTusk JVM recursive-extraction worker.

    Implements the ``detonator.worker.engine.Engine`` protocol.
    """
    ...
    def detonate(
        ...
        1. Run the ``redtusk-worker:default`` Docker container, writing rmeta
           ...
           ``metadata.json`` does not collide with the detonator harness's own
        ...
        # ``metadata.json`` (rmeta format) never clobbers the detonator
        ...
        # detonator envelope path confinement passes with outdir as root.
