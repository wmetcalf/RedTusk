...
def test_classify_mismatch() -> None:
    status, value = p.classify(_REAL_MISMATCH)
    assert status == p.MISMATCH
    assert value == "0x102100055bbd7,0x1c8"


def test_classify_compatible_on_success_marker() -> None:
    assert p.classify(_COMPATIBLE) == (p.COMPATIBLE, None)


def test_classify_inconclusive_without_success_or_mismatch() -> None:
...
def test_classify_custom_ok_marker() -> None:
...
def test_main_rejects_invalid_marker() -> None:
