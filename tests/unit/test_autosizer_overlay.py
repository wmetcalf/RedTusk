...
def test_no_service_hardcodes_an_image_tag(overlay: dict) -> None:
...
def test_the_api_is_not_given_a_pool_identity(overlay: dict) -> None:
...
def test_every_dispatcher_participates(overlay: dict) -> None:
...
def test_every_participant_mounts_the_shared_view(overlay: dict) -> None:
...
def test_the_declared_redtusk_ram_matches_what_a_guest_gets() -> None:
    ...
    assert m.group(1) == FLEET_WIDE["BLASTBOX_NODE_ENGINE_REDTUSK_RAM_MIB"], (
        f"guests get {m.group(1)} MiB but the balancer is told "
        f"{FLEET_WIDE['BLASTBOX_NODE_ENGINE_REDTUSK_RAM_MIB']}"
    )
