...
def test_the_declaration_is_readable_and_covers_every_tier() -> None:
    ...
    assert {"redtusk-warm-gvisor", "redtusk-fc-worker"} <= names, "a warm tier is undeclared"
...
class TestAFailedVersionCannotSatisfyTheFloor:
    ...
    def test_the_wrapper_runs_in_a_shell_where_BASHPID_is_unset(self, stub_cli: Path) -> None:
        ...
        reference. So the failure mode is reproducible on this host, and the fix can be
        proven by running the wrapper instead of reading it. Measured with the guard
        reverted to a bare `$BASHPID`:
